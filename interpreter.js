...
Interpreter.prototype.initFunction = function (globalObject) {
  ...
  wrapper = function Function(var_args) {
    ...
    return thisInterpreter.createFunction(
      node,
      thisInterpreter.globalScope,
      "anonymous",
    );
  };
...
Interpreter.prototype.initObject = function (globalObject) {
  ...
  wrapper = function getOwnPropertyDescriptor(obj, prop) {
    ...
    var pseudoDescriptor = thisInterpreter.createObjectProto(
      thisInterpreter.OBJECT_PROTO,
    );
    if (getter || setter) {
    ...
    if (thisInterpreter.isa(separator, thisInterpreter.REGEXP)) {
      ...
      if (thisInterpreter["REGEXP_MODE"] === 2) {
        if (Interpreter.vm) {
          ...
          var jsList = thisInterpreter.vmCall(
            code,
            sandbox,
            separator,
            callback,
...
Interpreter.prototype.initError = function (globalObject) {
  ...
  var createErrorSubclass = function (name) {
    var constructor = thisInterpreter.createNativeFunction(function (
      opt_message,
    ) {
      if (thisInterpreter.calledWithNew()) {
  ...
  for (var i = this.stateStack.length - 1; i >= 0; i--) {
    ...
    if (node.type === "CallExpression") {
      ...
      if (func && tracebackData.length) {
        tracebackData[tracebackData.length - 1].datumName = this.getProperty(
          func,
          "name",
        );
      }
 ...
 */
Interpreter.prototype.vmCall = function (
  code,
  sandbox,
  nativeRegExp,
  callback,
...
Interpreter.prototype.maybeThrowRegExp = function (nativeRegExp, callback) {
  ...
  if (!ok) {
    ...
    this.throwException(
      this.ERROR,
      "Regular expressions not supported: " + nativeRegExp,
    );
  }
};
 ...
 */
Interpreter.prototype.regExpTimeout = function (
  nativeRegExp,
  worker,
  callback,
) {
 ...
 * @private
 */
Interpreter.prototype.createFunctionBase_ = function (
  argumentLength,
  isConstructor,
) {
...
Interpreter.prototype.getProperty = function (obj, name) {
  ...
  if (typeof obj === "object" && !(obj instanceof Interpreter.Object)) {
    // CHANGE : throw TypeError('Expecting native value or pseudo object');
    return obj[name];
  }
  if (name === "length") {
 ...
 */
Interpreter.prototype.setProperty = function (
  obj,
  name,
  value,
  opt_descriptor,
 ...
 */
Interpreter.prototype.setNativeFunctionPrototype = function (
  obj,
  name,
  wrapper,
) {
 ...
 * @param {!Function} wrapper Function object.
 */
Interpreter.prototype.setAsyncFunctionPrototype = function (
  obj,
  name,
  wrapper,
  nArgs = -1,
    ...
    switch (state.node.type) {
      ...
      case "NewExpression":
        if (type === Interpreter.Completion.RETURN) {
          ...
          type === Interpreter.Completion.BREAK ||
  ...
  if (this.class === "Array") {
    ...
    try {
      ...
      for (var i = 0; i < maxLength; i++) {
        ...
        strs[i] =
          value instanceof Interpreter.Object && cycles.indexOf(value) !== -1
            ? "..."
            : value;
      }
      if (truncated) {
...
Interpreter.prototype["stepBinaryExpression"] = function (stack, state, node) {
  ...
  switch (node.operator) {
    ...
    case "instanceof":
      ...
      value =
        leftValue instanceof Interpreter.Object
          ? this.isa(leftValue, rightValue)
          : false;
      break;
    default:
  ...
  if (!state.doneExec_) {
    ...
    if (funcNode) {
      ...
      for (var i = 0; i < funcNode.params.length; i++) {
        ...
        var paramValue =
          state.arguments_.length > i ? state.arguments_[i] : undefined;
      ...
      var argsWithCallback = state.arguments_
        .concat(new Array(argLength))
        .slice(0, argLength);
      ...
      if ( state.funcThis_ instanceof Interpreter.Object && state.funcThis_.data) {
        result = func.asyncFunc.apply(state.funcThis_.data,argsWithCallback);
      } else {
        result = func.asyncFunc.apply(state.funcThis_,argsWithCallback);
      }
      return;
    } else {
      /* CHANGE - BEGIN  */
      ...
      var args = [];
      for (var i = 0; i < state.arguments_.length; i++) {
        var arg = state.arguments_[i];
        if ( arg instanceof Interpreter.Object ) {
          if ( arg.data ) 
            arg = arg.data
          else 
            arg = this.pseudoToNative(arg);
        }
        args.push(arg);
      }
      var result;
      if (  state.funcThis_ instanceof Interpreter.Object && state.funcThis_.data) {
        result = func.apply(state.funcThis_.data,args);
      } else {
        result = func.apply(state.funcThis_,args);
      }
    ...
    gotPropName: while (true) {
      if (state.object_ instanceof Interpreter.Object) {
        ...
        while (true) {
          ...
          state.visited_[prop] = true;
          if (
            !Object.prototype.propertyIsEnumerable.call(state.object_, prop)
          ) {
            continue; // Skip non-enumerable property.
          }
          state.name_ = prop;
